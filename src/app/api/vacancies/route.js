    ...
    return await runWithSessionStore(sessionId, async () => {
      ...
      if (type === 'predicted') {
        vacancies = vacancies.filter(v =>
          ['EXPECTED', 'CONFIRMED_INTENT'].includes(v.status)
        );
      } else if (type === 'verified') {
        vacancies = vacancies.filter(v =>
          ['VERIFIED_VACANCY', 'ALLOCATION_RECOMMENDED'].includes(v.status)
        );
      }
      ...
      const enriched = vacancies.map(v => {
        ...
        const fromStation = store.stations.find(
          s => s.code === v.fromStation
        );

        const toStation = store.stations.find(
          s => s.code === v.toStation
        );
