  ...
  try {
    ...
    if (!from) {
      return NextResponse.json(
        { success: false, error: 'Origin station required' },
        { status: 400 }
      );
    }
    ...
    if (!to) {
      return NextResponse.json(
        { success: false, error: 'Destination station required' },
        { status: 400 }
      );
    }
    ...
    return await runWithSessionStore(sessionId, async () => {
      ...
      if (!validCodes.includes(from)) {
        return NextResponse.json(
          {
            success: false,
            error: 'Invalid origin station'
          },
          { status: 400 }
        );
      }

      if (!validCodes.includes(to)) {
        return NextResponse.json(
          {
            success: false,
            error: 'Invalid destination station'
          },
          { status: 400 }
        );
      }

      const trains = searchTrains(
        from,
        to,
        date,
        cls,
      ...
      const results = trains.map(t => {
        ...
        if (
          t.availability.status !== 'CONFIRMED' ||
          t.availability.count === 0
        ) {
          smartAlternates = getSmartAlternates(
            from,
            to,
            cls,
            date,
      ...
      const globalSmartAlternates = getSmartAlternates(
        from,
        to,
        cls,
        date,
